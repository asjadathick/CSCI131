...
var executionmode; // Single step, or step-step
var halted;
var timer1; // timer for step-step
var displaylinks; // Array indexed by address gives byte where source code starts
var intcount = 0;
// traps
var
        INTBUS = 0004,
...
{
    if (a & 1)
        throw Trap(INTBUS, "read from odd address " + ostr(a, 6));
    if (a < 0760000)
        return memory[a >> 1];
    if (a === 0777546)
        return LKS;
...
{
    if (a % 1)
        throw Trap(INTBUS, "write to odd address " + ostr(a, 6));
    var str = octstrprint(a);
    if (a < 0760000)
        memory[a >> 1] = v;
    else if (a === 0777776) {
        switch (v >> 14) {
        ...
        step()
{
    ...
    switch (instr & 0177000) {
        ...
        case 0072000: // ASH
            ...
            if (val & 0100000)
                PS |= FLAGN;
            if (xor(val & 0100000, val1 & 0100000))
                PS |= FLAGV;
            return;
        case 0073000: // ASHC
            val1 = (R[s & 7] << 16) | R[(s & 7) | 1];
            ...
            if (val & 0x80000000)
                PS |= FLAGN;
            if (xor(val & 0x80000000, val1 & 0x80000000))
                PS |= FLAGV;
            return;
        case 0074000: // XOR
            val1 = R[s & 7];
            ...
            memwrite(da, 2, val);
            return;
 	case 0075000: // Non existent instruction - it corresponds to FADD in the KE11-f option
            // trick to switch from step-step to single step mode
            if(executionmode==="stepstep") {
		 alert("Invalid extended arithmetic instruction FADD (KE11-f option) - trick to set simulator in single step mode!");
	    	 executionmode= "step";
		}
            return;
        case 0077000: // SOB
    ...
    switch (instr & 0177700) {
        ...
        case 0000300: // SWAB
            ...
            memwrite(da, l, val);
            return;
            // AIJU added mark at my request - with my further correction
        case 0006400: // MARK
            R[6] += ((instr & 077)+1) << 1;
            R[7] = R[5];
            R[5] = pop();
    ...
    switch (instr & 0177400) {
        ...
        case 0002400:
            ...
            return;
        case 0003000:
            if (!xor(PS & FLAGN, PS & FLAGV) && !(PS & FLAGZ))
                branch(o);
            return;
        case 0003400:
            if (xor(PS & FLAGN, PS & FLAGV) || (PS & FLAGZ))
    ...
    switch (instr) {
        case 0000000: // HALT
            alert("Halt instruction reached");
            halted = true;
            if (curuser)
                break;
            writedebug("HALT\n");
        ...
        case 0000001: // WAIT
            ...
            waiting = true;
            return;
            // RTI and RTT have same code
            //nabg modification so as to set breakpoints when debugging demo os
        case 0000002: // RTI
         R[7] = pop();
            val = pop();
            if (curuser) {
        ...
        reset()
{
    ...
    for (i = 0; i < memory.length; i++)
        memory[i] = 0;
    // Setting of bootrom omitted for demo (not loading Unix)
//    for (i = 0; i < bootrom.length; i++)
//        memory[01000 + i] = bootrom[i];
    ...
    cleardebug();
    clearterminal();
    // Reset of rk device omitted for demo
    // rkreset();
    clkcounter = 0;
    waiting = false;
}
...
function loader(data) {
    ...
    for (i = 0; i < nwords; i++) {
        ...
        var word = obj.value;
        displaylinks[addr] = obj.linelocation;
        // Fix address
        addr = addr >> 1;
if(memory[addr]!=0) {
	alert("Overwriting code or data, check your addresses");
	break;
}
        memory[addr] = word;
...
function initforsteprun(startaddr) {
    R[7] = startaddr;
    R[6] = 512;
    displayR0 = document.getElementById("dr0");
    displayR1 = document.getElementById("dr1");
    displayR2 = document.getElementById("dr2");
    displayR3 = document.getElementById("dr3");
    displayR4 = document.getElementById("dr4");
    displayR5 = document.getElementById("dr5");
    displayR6 = document.getElementById("dr6");
    displayR7 = document.getElementById("dr7");

    chkN = document.getElementById("statusN");
    chkZ = document.getElementById("statusZ");
    chkV = document.getElementById("statusV");
    chkC = document.getElementById("statusC");
    priotxt = document.getElementById("prio");
    //displayinstruction = document.getElementById("theinstruction");
    displaymemdump = document.getElementById("memdump");
    halted = false;
    showregisters();
}


function displayline(pc1) {
    // Have problem if executing an instruction that has
    // been generated and stored - there won't be any display
    // code.
    // Only case expected is "mark" instructions in the stack
    // Just don't try to display these
    // Recognize by an address on stack
    if (pc1 < 01000)
        return;
    var startloc = displaylinks[pc1];
    if (startloc === undefined) {
        alert("Jump to uninitialized memory - oops");
        executionmode = "none";
        halted = true;
        return;
    }
    var target = document.getElementById('gencode'); // textarea
    var displaytxt = target.value;
    var endloc = startloc;

    while (displaytxt.charAt(endloc) !== '\n')
        endloc++;

    target.selectionStart = startloc;
    target.selectionEnd = endloc;

    var height = target.clientHeight;

    var lineheight = height / target.rows;

    var currentrow = 0;
    var aloc = 0;
    while (aloc < startloc) {
        if (displaytxt.charAt(aloc) === '\n')
            currentrow++;
        aloc++;
    }

    var rowheight = lineheight * currentrow;

    var currenttop = target.scrollTop;

    var currentbottom = currenttop + height;

    if ((rowheight > currenttop) && (rowheight < currentbottom))
        return;


    // Need to scroll 
    target.scrollTop = rowheight - 2 * lineheight;



}

function checkthoseinterrupts() {
    if (interrupts.length && interrupts[0].pri >= ((PS >> 5) & 7)) {
                handleinterrupt(interrupts[0].vec);
                interrupts.splice(0, 1);
            }
...
function displaystep() {
    ...
    try {
        step();
        checkthoseinterrupts();
    } catch (err) {
        if (err instanceof Trap) {
            alert("Trap #" + err.num + ", " + err.msg);
        }
        else {
            alert("Error" + err);
            alert(err.stack);
    ...
    showregisters();
    if ((executionmode === "stepstep") & !halted)
        timer1 = setTimeout(displaystep, 20);
}
...
function step1() {
    ...
    try {
        clearTimeout(timer1);
        executionmode = "step";
        //var instr = disasm(R[7]);
        // displayinstruction.innerHTML = instr;
        displaystep();
    }
...
function stepstep() {
    executionmode = "stepstep";
    halted = false;
    timer1 = setTimeout(displaystep, 20);
}

function showregisters() {
    displayR0.innerHTML = octstrprint(R[0]);
    displayR1.innerHTML = octstrprint(R[1]);
    displayR2.innerHTML = octstrprint(R[2]);
    displayR3.innerHTML = octstrprint(R[3]);
    displayR4.innerHTML = octstrprint(R[4]);
    displayR5.innerHTML = octstrprint(R[5]);
    displayR6.innerHTML = octstrprint(R[6]);
    displayR7.innerHTML = octstrprint(R[7]);
    ...
    var priority = PS & 0340;
    priority = priority >> 5;
    priotxt.value = priority;
}

function dumpmemory() {
    displaymemdump.value = "";
    var length = memory.length;
    var i;
    var dotsprinted = false;
    for (i = 0; i < length; i += 8) {
        var data = false;
        var j;
        for (j = i; j < i + 8; j++)
            data = data | (memory[j] !== 0);
        if (data) {
            dotsprinted = false;
            var str = octstrprint(i * 2);
            for (j = i; j < i + 8; j++)
                str = str + "  " + octstrprint(memory[j]);
            displaymemdump.value =
                    displaymemdump.value + str + "\n";
        }
        else
        if (!dotsprinted) {
            dotsprinted = true;
            displaymemdump.value = displaymemdump.value +
                    "  ...       ...    ....\n";
        }
